package com.karthika.bloodbank.Search;// update info
...
import com.karthika.bloodbank.Donor.DonorLogin;
...
public class Update extends JFrame implements MouseListener, ActionListener {
	...
	//public static com.arnabb.bbms.Search.Update regRef = null;
	...
	public Update(String lID) {
		...
		updateButton = new JButton("com.karthika.bloodbank.Search.Update");
						...
						if(mobTF.getText().length()!=0 && mobTF.getText().length()==11 ) {
							...
							if(mailTF.getText().length()!=0 && mailTF.getText().matches("(.*)@(.*).com(.*)") == true ) {  
								...
								if(passTF.getText().length()>=5) {
									...
									JOptionPane.showMessageDialog(null, "com.karthika.bloodbank.Search.Update Succeed!");
